...
- **Type**: scrutiny | behavioral | manual | browser
...
`browser` assertions are for UI features only: the **Verify** command must actually launch a
headless browser and capture evidence -- `python3 execution/browser_qa.py --url <url>
--screenshot <path> [--assert-text ...] [--assert-selector ...]` -- never a manual "open this in a
browser and look" instruction. @qa executes these directly over its existing Bash tool; see
`.agent/agents/qa.md`. Skip this Type entirely for projects/features with no UI (`project_type`
empty or CLI/backend-only) -- it is opt-in per assertion, not a default requirement.
