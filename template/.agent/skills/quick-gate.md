...
## What it does
Reads `active.json` → derives slug → finds matching `contract-f*.yaml` (feature-scoped; falls back to legacy `contract-f1.yaml`) → runs `contract.py gate --phase 4 --run-checks`
