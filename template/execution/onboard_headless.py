...
def main() -> None:
    ...
    if not args.allow_harness_rewrite and os.path.exists(args.workspace_path):
        ...
        effective_harness_name = DEFAULT_HARNESS_NAME
        if os.path.exists(args.profile_path):
            with open(args.profile_path, "r") as fh:
                existing_profile = json.load(fh)
            effective_harness_name = existing_profile.get("harness_name", DEFAULT_HARNESS_NAME)
