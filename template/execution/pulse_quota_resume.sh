...
#
# ADDITIVE (mission quota-aware-pause-resume F2): when the live oracle
# above is unusable -- which it always is once the session has been paused
# long enough for the mirror to go stale -- this falls back to the
# checkpoint's own recorded resets_at via execution/quota_resume_window.py.
# The flag gate above stays the first thing this script does, and the
# `quota.py status` call and shared ticket writer below are unchanged; the
# fallback only ever fires with the flag explicitly ON, same as everything
# else in this file.
set -u
...
now_epoch="$(date -u +%s)"
if [ -z "$resets_epoch" ] || [ "$now_epoch" -lt "$resets_epoch" ]; then
    # --- 5b. FALLBACK (mission quota-aware-pause-resume F2, D-6) -----------
    # The live oracle above is unusable in exactly the scenario this script
    # exists for: a paused session produces no turns, so the mirror is
    # guaranteed stale within quota.py's STALE_SECONDS and `state` reports
    # unknown. Fall back to the resets_at recorded in the checkpoint itself
    # by the sanctioned reader (F1/inject_pressure.sh) at pause time, via
    # the single shared resolver (execution/quota_resume_window.py) so this
    # script and execution/quota_resume_notice.sh cannot drift on the
    # precedence logic.
    ...
    resolver_verdict="$(printf '%s' "$resolver_out" | awk '{print $1}')"
    [ "$resolver_verdict" = "READY" ] || exit 0
fi

# --- 6. Deterministic dedupe_key, derived from the checkpoint's own ts -----
dedupe_key="quota-resume:${PROJECT_PATH}:${checkpoint_ts}"

# --- 6b. Skip if a ticket for this exact pause is already queued -----------
# pulse_ticket.py itself does not dedupe at enqueue time — that lives in
# pulse_dispatcher.py's dedupe_active/reserve_dedupe TTL, which acts at
# DISPATCH time (F4/A9) and is unaffected by this check. But a Pulse job
# ticking on a fixed cadence must not flood the queue with N copies of the
# same resume ticket while the first one waits to be picked up, so guard at
# enqueue time too, keyed on the same dedupe_key.
effective_queue_dir="$QUEUE_DIR"
if [ -z "$effective_queue_dir" ]; then
    effective_queue_dir="$(python3 -c '
import sys
from pathlib import Path
print(Path(sys.argv[1]).expanduser().resolve() / ".agent" / "pulse" / "queue")
' "$PROJECT_PATH" 2>/dev/null)"
fi
if [ -n "$effective_queue_dir" ] && [ -d "$effective_queue_dir" ]; then
...
try:
    for f in Path(sys.argv[1]).glob("*.json"):
        try:
            ...
            continue
        if data.get("dedupe_key") == sys.argv[2]:
            print("1")
            break
except Exception:
    pass
' "$effective_queue_dir" "$dedupe_key" 2>/dev/null)"
    [ "$already_queued" = "1" ] && exit 0
fi
