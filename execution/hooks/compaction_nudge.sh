...
# written by mission.py's emit_compaction_hint() at checkpoint/gate/close-out.
# If a hint file exists, surface it as additionalContext telling Brad this is
# a good moment to run /compact, then delete the file (consumed at most once,
# whether or not it was usable). If absent or corrupt, emit an empty JSON
# object and exit 0 — this hook runs on every user turn and must never block
# a turn. Mirrors post_compact_inject.sh's flag-file pattern.
set -uo pipefail
