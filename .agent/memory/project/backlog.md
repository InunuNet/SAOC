...
- [ ] **Auto-refresh llms-full.txt via GitHub Actions + Sanity GROQ** — nightly cron queries Sanity GROQ API directly (no Alembic/screen-scraping), commits updated public/llms-full.txt, Firebase auto-deploys. Depends on: DNS cutover, Sanity API token in GitHub secrets.
