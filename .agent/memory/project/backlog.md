...
- [ ] **Phase A next: A6 next-sanity wiring with draft mode** — `sanity/lib/fetch.ts` tagged fetches for ISR, draft-mode toggle route (`app/api/draft/route.ts`), `sanity/queries.ts` reusable GROQ. Gate: home page renders Sanity data; type-check clean.
