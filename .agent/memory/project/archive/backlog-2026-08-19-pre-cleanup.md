...
- [ ] SAOC (Ticketing): F4 door check-in is NOT proven live — ticket SAOC-2027-X8ZPQNYCVWGY still status='paid', zero checkinAttempts docs. Brad runs docs/f4-door-checkin-manual-protocol.md to close it. Mission F4 status corrected done->pending 2026-08-19.
- [ ] SAOC (Harness): contract assertions read $MISSION_F2_BOOKING_REF, which nothing sets — mission.py cmd_gate and contract.py both pass a plain env copy. Gate silently runs with an empty ref and fails for the wrong reason. Persist the ref or inline it into the assertion command.
- [ ] SAOC (Harness): specs/prove-ticket-purchase-works-end-to-end-b/contract-f1.yaml is one omnibus file holding all 10 assertions for F1-F4. No feature declares a contract: field, so gating M1 evaluates F3/F4 assertions outside that milestone and skips F2 entirely. Split per feature.

## Session 2026-08-19 — Stale sha256 pins on the ITN route (found by @architect, F1 payment seam)

- [ ] **[P1, NEW 2026-08-19, agent-actionable, BLOCKS mission `payment-provider-seam` F2] All four
  sha256 pins of `app/api/tickets/itn/route.ts` are stale.** Actual file is `a71f9505...`;
  `ticketing-f1-show-collision` and `ticketing-m1-m2` pin `253c15c4...`, `ticketing-f10-itn-repin`
  and `ticketing-hardening` pin `553f67d8...`. Drift is almost certainly the 2026-08-18 source-IP
  "logged, not enforced" change — documented in the route's own comment but never re-pinned.
  Two consequences: four contracts carry a red assertion nobody is watching, independent of any
  mission; and F2 reopens that exact file. **Decision taken 2026-08-19 (orchestrator): the current
  content IS the intended baseline — the drift is the reviewed and documented source-IP change, not
  an unreviewed edit — so F2's contract must plan a re-pin ceremony (architect authors the expected
  value, @dev never computes a pin) rather than treat it as a regression.** A re-pin is a ceremony,
  not an in-passing edit; @architect correctly declined to do it inline.
