...
## Phase A6 — next-sanity wiring + draft mode (2026-06-08)

- (2026-06-08) **`createClient({ projectId: '' })` throws at build time:** `@sanity/client` validates `projectId` eagerly in the constructor — empty string → "Configuration must contain `projectId`". `pnpm build` runs without `NEXT_PUBLIC_SANITY_PROJECT_ID` in CI and explodes. The ONLY safe pattern: type the export as `SanityClient | null`, return `null` from the factory when `projectId` is empty, and null-check at every call site. Guarding inside the fetch wrapper alone is too late — the module top-level `createClient(...)` call already threw.
- (2026-06-08) **`draftMode()` in Next 15 is async AND request-scoped:** Must `await draftMode()` (vs the sync v14 API) AND must be called only inside a real request context. If called during `pnpm build` static analysis or in a non-request module init path, it throws "Dynamic server usage" / "called outside request scope". In `sanityFetch`, call it AFTER the null-client guard so build-time evaluation short-circuits before hitting the async API.
- (2026-06-08) **`defineQuery` lives in `next-sanity`, not `sanity`:** Import `import { defineQuery } from 'next-sanity'`. This is the marker that `@sanity/codegen` looks for when generating typed GROQ result types — using the `sanity` package's re-export works at runtime but breaks future codegen pipelines. Apply consistently across all `sanity/queries.ts` constants.
- (2026-06-08) **GROQ type names must match schema `name:` exactly:** Events use `_type == "societyEvent"` (not `"event"`) — the schema file is `sanity/schemas/documents/event.ts` but its `defineType({ name: 'societyEvent', ... })` is what GROQ filters on. Always cross-check the `name:` field of the schema, not the filename or the TS export. Wrong type name = silent empty result set, not an error.
- (2026-06-08) **ISR cache tags via `next: { tags: ['sanity'] }` + `revalidateTag`:** Tag every Sanity fetch with `'sanity'` (broad) plus a granular tag (e.g. `sanity:home`). Webhook route `app/api/revalidate/route.ts` validates a shared secret then calls `revalidateTag('sanity')` to nuke the whole CDN cache on any Studio publish. Keeps Studio→site latency near-zero without per-doc routing logic.

## Phase A7 — Firebase App Hosting scaffolding (2026-06-08)

- (2026-06-08) **`apphosting.yaml` `env` block: `secret: true` and `availability` are mutually exclusive.** Setting both on the same env entry fails App Hosting validation. For Sanity secrets, use `secret: true` alone (pulled from Google Secret Manager); for plain build/runtime vars use `value:` + `availability: [BUILD, RUNTIME]`. Do not mix.
- (2026-06-08) **Firebase App Hosting `outputFiles` must include `next.config.ts` for Next.js 15.** Just listing `package.json` is not enough — the build server needs the Next config in the output bundle or Next 15's App Router runtime misbehaves on cold start. Pattern: `outputFiles: { serverApp: { include: [package.json, next.config.ts] } }`.
