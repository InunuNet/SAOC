 ...
 *
 * The gateway POSTs application/x-www-form-urlencoded on payment completion. Every check
 * below must pass before an order is ever flipped to 'paid'. Any single failure logs
 * which check failed and leaves the order untouched — we still return HTTP 200 so the
 * gateway stops retrying, but a 200 response here never implies the payment was accepted.
 *
 * F2 (payment-provider-seam) — this route no longer knows which gateway it is talking to.
 * Every gateway-specific step (parsing the posted body, authenticating it against the shared
 * secret, resolving and judging the source IP, the out-of-band server-confirm round trip, and the
 * gateway's own status vocabulary) now lives behind the PaymentProvider interface
 * (lib/payments/). What stays here is everything that is OURS and cannot move without a
 * behaviour change: the order lookup, the already-settled short circuit, the amount
 * comparison against our own stored figure, the atomic order+position write, and the
 * post-commit confirmation email. See contracts/golden/payment-seam-f2/README.md for the
 * step-by-step mapping from the pre-F2 sequence to this one, and for why each of those
 * five stayed.
 *
 * The ordering of the eleven steps is unchanged from the pre-F2 route and is load-bearing:
 * verification, then source-IP logging, then reference, then lookup, then the settled
 * short circuit, then the amount match, then the server confirmation, then the status
 * check, then the write, then the email. Nothing here may be reordered for tidiness.
 *
