...
- **GitHub Issue #1272 (mission.py new exits 0 on stale slug match instead of continuing)**: This issue has been resolved. The `execution/mission.py` file has been modified to ensure that `cmd_new` now correctly exits with `sys.exit(1)` and an error message if any mission with the same slug exists, regardless of its status. This change was verified by updating `execution/tests/test_mission.py` to reflect the new expected behavior, and all tests now pass. No new mission file is created on slug collision.

- **GitHub Issue #1274 (bug: require_docs.sh hook substring-matches any command containing gate keyword)**: This issue has been resolved and verified. The `execution/hooks/require_docs.sh` script's `case` statement was updated to use strict prefix matching for "contract.py gate" commands, removing the leading wildcards. This prevents the hook from incorrectly activating on commands that merely contain "gate" as a substring. The change ensures the hook only triggers when the command explicitly starts with `contract.py gate` or is exactly `contract.py gate`. This has been re-verified by inspecting the file.
