...
| Verification Triad Gate M1-F1 | Mechanism done, mandate deferred (M2) | Adds `browser_deployed_check` and `gws_inbox_check` as first-class contract assertion kinds, sibling to `codex_qa`, plus a triad-coverage linter — closing the mechanism gap that let the 2026-09-01 SITE_URL/hosted.app defect reach a 10/10 green gate with zero deploys and zero real browser/inbox verification. The coverage linter is not yet wired into any gate path, so declaring the triad on a contract is still voluntary. See [docs/verification-triad-gate.md](docs/verification-triad-gate.md). |
