...
## A6 — next-sanity Wiring

Connects the Next.js app to Sanity with server-side data fetching, draft mode, and ISR cache invalidation.

### Files added

| File | Purpose |
| --- | --- |
| `sanity/lib/client.ts` | Sanity client (`SanityClient \| null`); returns `null` when `NEXT_PUBLIC_SANITY_PROJECT_ID` is unset so CI builds pass before Sanity is provisioned |
| `sanity/lib/fetch.ts` | `sanityFetch<T>()` — server utility; switches `perspective` to `previewDrafts` when `draftMode().isEnabled`; returns `null` on missing client or error |
| `sanity/queries.ts` | 5 GROQ constants via `defineQuery`: `homePageQuery`, `societyListQuery`, `nationalShowQuery`, `upcomingEventsQuery`, `partnersQuery` |
| `app/api/draft/route.ts` | GET — validates `?secret=`, enables Next.js draft mode |
| `app/api/disable-draft/route.ts` | GET — disables draft mode, redirects to `/` |
| `app/api/revalidate/route.ts` | POST — validates secret (header or query), calls `revalidateTag('sanity')` for ISR invalidation |
| `app/(marketing)/page.tsx` | Now async; calls `sanityFetch({ query: homePageQuery, tags: ['homePage', 'sanity'] })`. Component prop-wiring deferred to Phase B |

### Cache tag strategy

Every fetch is tagged `'sanity'` globally plus a per-type tag (e.g. `'homePage'`). The webhook at `/api/revalidate` calls `revalidateTag('sanity')` to bust the ISR cache on any CMS edit.

### Draft mode

The Sanity Studio Presentation tool enables draft mode via:

```
GET /api/draft?secret=<SANITY_REVALIDATE_SECRET>
```

Disable via `GET /api/disable-draft`. When draft mode is active, `sanityFetch` switches `perspective` to `previewDrafts` so editors see unpublished content.

### Environment variables added to `.env.local.example`

| Variable | Notes |
| --- | --- |
| `SANITY_REVALIDATE_SECRET` | Shared secret for the revalidation webhook and draft mode toggle |
| `SANITY_API_TOKEN` | Read token for server-side fetches (Viewer permissions sufficient) |

### Contract gate

14/14 assertions pass (A6a–A6h structural checks, A6i type-check, A6j build).

### Known follow-up (Phase B)
