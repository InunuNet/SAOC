...
- **`contracts/golden/f4-seed-page-singletons/nationalShow.golden.json`** still pins CTICC as expected output (owned by `contracts/cms-loop-f3-national-show.yaml`). This contract does not touch it; resetting it is the F3 contract's job and is self-detecting on its next run.

- **Older golden files** (`cticc-research.golden.md`, `venue-single-source.golden.md`, `show-identity-wiring.golden.md`, `assertion-discrimination.golden.md`) are classified as historical research records and left intact. These document what was researched when for the *old* venue. Decide whether they should carry a "superseded" banner or be archived.
